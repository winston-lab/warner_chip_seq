...
with open(r"logs/merged_experimental_counts.log", 'r') as fp:
...
for i in range(0, lala, 2):
    file = open('logs/merged_experimental_counts.log', 
                mode='r')
    content = file.readlines()
    libraries.append(content[i].strip('\n'))
    scer_counts.append(int(content[i+1].strip('\n')))
    file.close()
    file = open('logs/merged_spikein_counts.log', 
                mode='r')
    content = file.readlines()
...
plt.savefig('logs/merge_proportion_reads_mapped.png', dpi=300)
...
aligned_reads[["library", "alpha_IP_float"]].to_csv(path_or_buf='logs/merged_normalization_table.csv', index=False, header=False)
